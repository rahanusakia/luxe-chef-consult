...
const PageHeader = ({ title, subtitle, backgroundImage, breadcrumbs }: PageHeaderProps) => {
  return (
    <div 
      ...
      <div className="relative z-10 container mx-auto px-6 text-center">
        <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">
          {title.split(" ").map((word, i, arr) => 
            i === arr.length - 1 ? 
              <span key={i}><span className="text-gold-light">{word}</span></span> : 
              <span key={i}>{word} </span>
          )}
        </h1>
        {subtitle && (
          <p className="text-white/70 max-w-3xl mx-auto mb-6">{subtitle}</p>
        )}
        <div className="flex items-center justify-center text-sm md:text-base text-white/80 space-x-2">
          {breadcrumbs.map((item, index) => (
            <span key={index} className="flex items-center">
              {index > 0 && <ArrowRight size={14} className="text-gold-light mx-2" />}
              {item.isCurrent ? (
                <span className="text-gold-light">{item.label}</span>
              ) : (
                <Link to={item.href} className="hover:text-gold-light transition-colors">
                  {item.label}
                </Link>
              )}
            </span>
